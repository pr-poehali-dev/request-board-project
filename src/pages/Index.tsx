    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50/50 via-pink-50/50 to-orange-50/50">
      <nav className="bg-gradient-to-r from-gray-900 via-gray-800 to-black border-b border-gray-700/50 sticky top-0 z-50 shadow-2xl backdrop-blur-lg">
        <div className="container mx-auto px-3 sm:px-6 lg:px-8" style={{ maxWidth: '1400px' }}>
          ...
          <div className="hidden lg:flex gap-3 pt-4 flex-wrap items-center justify-between border-t border-white/20">
            <div className="flex gap-2">
              <button
                onClick={() => setActiveTab('requests')}
                className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${
                  activeTab === 'requests' ? 'bg-white text-primary' : 'text-white/80 hover:bg-white/10'
                }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${
                  activeTab === 'offers' ? 'bg-white text-primary' : 'text-white/80 hover:bg-white/10'
                }`}
              >
            ...
            <div className="flex gap-2 items-center">
              <span className="text-xs text-white/70 mr-1">Сортировка:</span>
              <button
                onClick={() => setSortBy('date')}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  sortBy === 'date' ? 'bg-white text-primary' : 'text-white/80 hover:bg-white/10'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  sortBy === 'popular' ? 'bg-white text-primary' : 'text-white/80 hover:bg-white/10'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  sortBy === 'price' ? 'bg-white text-primary' : 'text-white/80 hover:bg-white/10'
                }`}
              >
                <Icon name="DollarSign" size={13} className="inline mr-1" />
                По цене
              </button>
            </div>
            ...
            <div className="flex gap-2 items-center">
              <div className="relative">
                ...
                <select
                  ...
                  className="appearance-none pl-9 pr-8 py-1.5 border border-white/30 bg-white/10 backdrop-blur-md rounded-lg focus:outline-none focus:ring-2 focus:ring-white/50 text-xs text-white cursor-pointer"
                >
                ...
                <Icon name="ChevronDown" size={14} className="absolute right-2 top-1/2 -translate-y-1/2 text-white/70 pointer-events-none" />
              </div>
            </div>
          </div>

          <div className="hidden lg:flex gap-2 pt-3 flex-wrap border-t border-white/20 mt-3">
            <button
              onClick={() => setSelectedCategory(null)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                selectedCategory === null ? 'bg-white text-primary' : 'text-white/80 hover:bg-white/10'
              }`}
            >
            ...
            {categories.map((category) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  selectedCategory === category.name ? 'bg-white text-primary' : 'text-white/80 hover:bg-white/10'
                }`}
              >
                <Icon name={category.icon as any} size={14} className="inline mr-1" />
                {category.name}
              </button>
            ))}
          </div>
